...
export default function ThemeList() {
	...
	const loadThemes = () =>
		apiFetch< Theme[] >( { path: '/clipisode/v1/themes' } ).then(
			setThemes
		);
	const loadPreviews = () =>
		apiFetch< PreviewLayout[] >( {
			path: '/clipisode/v1/preview-layouts',
		} ).then( setPreviews );

	useEffect( () => {
		Promise.all( [ loadThemes(), loadPreviews() ] ).finally( () =>
			setLoading( false )
		);
	}, [] );
	...
	const handleDeletePreview = ( id: number ) => {
		if (
			! window.confirm(
				'Delete this preview layout? This cannot be undone.'
			)
		) {
			return;
		}
		apiFetch( {
			path: `/clipisode/v1/preview-layouts/${ id }`,
			method: 'DELETE',
		} )
			.then( loadPreviews )
			.catch( ( err: { message?: string } ) =>
	...
	return (
		<>
					...
					{ themes.map( ( theme ) => (
						<div key={ theme.id } className="clipisode-theme-card">
							<div className="clipisode-theme-card-header">
								...
								{ theme.is_default && (
									<span className="clipisode-status-badge open">
										default
									</span>
								) }
							</div>
							<div className="clipisode-theme-card-meta">
								{ theme.topic_count }{ ' ' }
								{ theme.topic_count === 1 ? 'topic' : 'topics' }
							</div>
			...
			<div className="clipisode-settings-section">
				...
				<p style={ { color: '#646970', margin: '0 0 16px' } }>
					Controls the look and feel of public clipisode preview
					pages.
				</p>
				<div className="clipisode-theme-grid">
					{ previews.map( ( layout ) => (
						<div key={ layout.id } className="clipisode-theme-card">
							...
							<div className="clipisode-theme-card-actions">
								...
								{ renderCloneUI( `preview-${ layout.id }`, () =>
									handleClonePreview( layout.id )
								) }
								{ ! layout.is_default && (
									<Button
										variant="tertiary"
