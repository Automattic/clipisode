...
function UsedByCell( { item }: { item: MediaItem } ) {
	...
	return (
		<a href={ href }>
			<strong>{ prefix }:</strong> { usage.label }
			{ detail }
		</a>
	);
}
...
export default function MediaList() {
	...
	const isSelectable = ( item: MediaItem ) =>
	...
	return (
		<>
			...
			{ uploadError && (
				<div
					className="clipisode-video-error"
					...
					<tbody>
						{ items.map( ( item ) => (
							<tr key={ item.id }>
								...
								<td>
									<span
										className={ `clipisode-status-badge ${ item.type }` }
									>
										{ item.type }
									</span>
								</td>
